...
3. W oknie promptu Copilot CLI wyślij poniższe polecenie:
...
6. W nowej karcie przeglądarki wklej link, aby przejrzeć gist. Zauważ, jak gist podkreśla wysłane prompty, użyte skille i agentów, tok myślenia Copilota, a nawet kod i wyniki lokalnie uruchomionych poleceń.
...
1. W oknie promptu Copilot CLI wyślij poniższe polecenie:
...
6. Zauważ zmianę kontekstu. Może nie być drastyczna, ponieważ okno kontekstu jest obecnie prawdopodobnie stosunkowo małe.

> [!NOTE]
> Copilot CLI automatycznie kompaktuje kontekst, gdy się zapełni. Gdy zbliża się do 100% pojemności, wyświetla procent tuż nad oknem promptu. Zwykle kompaktuje asynchronicznie, pozwalając Ci dalej wchodzić w interakcję z Copilotem podczas pracy. Może jednak zablokować działającą operację na kilka sekund na czas kompaktowania.
...
3. Jeśli chcesz, wybierz nowy model! Albo naciśnij <kbd>Esc</kbd>, aby wyjść z listy modeli.
...
2. Wyślij mały, dobrze określony prompt. Na przykład możesz oddelegować paginację ze stretch goal z backlogu:
...
3. Wyślij poniższe polecenie slash, aby przekazać sesję do cloud agent, i potwierdź prompt, który chcesz oddelegować:
