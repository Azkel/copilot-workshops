...
W poprzedniej lekcji zapoznałeś się z obszarem roboczym i użyłeś szybkiego czatu. Teraz czas rozpocząć **sesję agenta** i wprowadzić pierwszą zmianę w projekcie. Zachowajmy ją małą: gry mają już ocenę gwiazdkową w danych, ale karty gier na stronie głównej jej jeszcze nie pokazują. Poprosisz agenta o jej wyświetlenie, przejrzysz zmianę i scalisz ją jako pierwszy pull request.
...
Każda gra w Tailspin Toys może mieć ocenę gwiazdkową i już pojawia się ona na stronie szczegółów gry. Karty gier na stronie głównej pokazują jednak tylko tytuł, kategorię, wydawcę i opis. Na rozgrzewkę agent wyświetli istniejącą ocenę na każdej karcie — drobna, samodzielna zmiana, idealna na pierwszą sesję.
...
Rozpocznijmy nową sesję, aby zbadać projekt i zaimplementować funkcję. W [poprzedniej lekcji][prior-lesson] dodałeś projekt z jego repozytorium na GitHubie. Utworzymy nową sesję dla tego repozytorium i poprosimy o zmianę.
...
3. Upewnij się, że dla repozytorium wybrane jest `tailspin-toys`.
   ...
   ![Pole monitu aplikacji GitHub Copilot z selektorem repozytorium ustawionym na tailspin-toys i selektorem modelu poniżej monitu](../../_images/app-2-start-session.png)

4. Użyj poniższego monitu, aby poprosić o zmianę:
...
5. Wybierz <kbd>Enter</kbd>, aby wysłać monit do Copilota.

Aplikacja Copilot zaczyna pracę od utworzenia nowego worktree — izolowanej kopii projektu. Następnie bada projekt, lokalizując pliki potrzebne do dodania nowej funkcji, i tworzy niezbędny kod. Właśnie dodałeś nową funkcję wykorzystując aplikację Copilot!
...
Oczywiście nie powinniśmy tylko czytać kodu i zakładać, że działa. Powinniśmy też przetestować wszystko wizualnie! W tym celu uruchomimy aplikację z terminala, a potem potwierdzimy, że wszystko działa. Na szczęście w aplikacji Copilot jest wbudowany terminal!
...
7. Wybierz <kbd>Ctrl</kbd>+<kbd>C</kbd>, aby zatrzymać serwer deweloperski.

## Otwórz i scal pierwszy pull request

Zmiana wygląda dobrze — czas ją wypchnąć! Poprosisz agenta o otwarcie pull requesta, a następnie sam go przejrzysz i scalisz na github.com. Na razie zrobimy to ręcznie. W nadchodzącej lekcji zobaczymy, jak Copilot może automatycznie przejąć część tej pracy.
...
W następnym kroku użyjesz aplikacji, aby dodać do repozytorium standard instrukcji niestandardowych — zaczynając od jednego ze zgłoszeń w backlogu. Przejdź do [Lekcji 3 - Prowadzenie Copilota instrukcjami niestandardowymi][next-lesson].
