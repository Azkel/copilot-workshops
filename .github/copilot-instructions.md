...
- `docs/` — **Source Markdown for all lessons. Edit here.** Browsable directly on github.com; no build required.
  ...
  - `es-es/`, `ja-jp/`, `ko-kr/`, `pl-pl/`, `pt-br/`, `zh-cn/` — Localized content at the locale-root paths required by Starlight. Translated pages mirror the English path beneath each locale directory; untranslated pages use Starlight's English fallback.
